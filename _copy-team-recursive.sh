#!/bin/bash
# NOTE: this script is called from other migration scripts, it's not meant to be called directly
if [ $# -lt 3 ]; then
...
debug "_copy-team-recursive.sh $source_org $target_org $slug parent=$parent_slug $parent_id"

# Get team details from source
SOURCE_TEAM_JSON=$(GH_TOKEN=$GH_SOURCE_PAT gh api "orgs/$source_org/teams/$slug" --jq '{name, description, privacy, notification_setting, parent_id: .parent.id, parent_slug: .parent.slug}')

# Check if team exists at target
# https://docs.github.com/en/rest/teams/teams?apiVersion=2022-11-28#get-a-team-by-name
if ! TARGET_TEAM_JSON=$(GH_TOKEN=$GH_PAT gh api "orgs/$target_org/teams/$slug" --jq '{name, description, privacy, notification_setting, id: .id, parent_id: .parent.id, parent_slug: .parent.slug}' 2>/dev/null); then
  ...
  new_team_id=$(GH_TOKEN=$GH_PAT gh api --method POST "orgs/$target_org/teams" --input - --jq .id <<<"$SOURCE_TEAM_JSON")
  echo "  Created team $slug ($new_team_id) $parent_desc"
  
  # Remove us from the team, so the team is empty
  if [ -z "$__ghuser" ]; then
    __ghuser=$(GH_TOKEN=$GH_PAT gh api user --jq '.login')
  fi
  GH_TOKEN=$GH_PAT gh api --method DELETE "orgs/$target_org/teams/$slug/memberships/$__ghuser" --silent 2>/dev/null || true
else
  ...
  if [ -n "$parent_id" ]; then
    if [ "$target_parent_id" != "$parent_id" ]; then
      if [ "$target_parent_id" != "null" ]; then
        ...
        GH_TOKEN=$GH_PAT gh api -X PATCH "orgs/$target_org/teams/$slug" \
...
GH_TOKEN=$GH_SOURCE_PAT gh api "orgs/$source_org/teams/$slug/teams" --jq '.[].slug' | while read -r child_slug; do
